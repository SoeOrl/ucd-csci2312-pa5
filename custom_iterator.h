...
namespace CS2312 {
    ...
    public:
        ...
        public:
			...
			self_type operator++(int junk) { self_type i = *this; __ptr++; return i; }
		...
		fixed_array(std::initializer_list<T> list) : __data(new T[list.size()]), __size(list.size())
		{
		...
		T& operator[](size_type index) { assert(index < __size); return __data[index]; }

        const T& operator[](size_type index) const { assert(index < __size); return __data[index]; }
